...
Fig. 3 renders the full pairwise κ heatmap. **Every off-diagonal cell is ≥ 0.56 — substantial or moderate by Landis-Koch.** Two clusters: reasoning-generous (Sonnet ↔ GPT-5.5 = 0.79 leads; ceiling 0.75-0.79) and strict-mid + open-weight (**Qwen ↔ Gemma 4 = 0.80 — matrix-highest**; Gemma 4 ↔ GPT-4o = 0.77; Qwen ↔ Opus 4.7 = 0.75). All three commercial within-family pairs (Anthropic 0.71, OpenAI 0.63, Google 0.67) sit **below** the cross-family reasoning ceiling (0.79); the fourth, cross-organization Open-weight pair, is the matrix maximum. Within-cluster mean κ (0.74) exceeds cross-cluster mean κ (0.68) by **0.06** (Welch t = 3.45, p = 0.002): **calibration philosophy partitions judges more reliably than provider lineage**, though the absolute gap is modest.

# 5. Findings (C1–C4)

**C1. Cross-family reasoning judges converge at κ ≥ 0.75.** **Nine cross-family pairs** reach substantial-or-better κ ≥ 0.75 (Fig. 3), spanning every pairwise combination of the five families — well above 0.4-0.6 typical [rahmani2024judges] and the 0.60 unweighted baseline of [thakur2025trecragsupport]. **DeepSeek V4 Pro joining the cluster makes a "Western training data" common-cause explanation less plausible,** and the pattern replicates across three independent ablation scales (5-, 7-, 9-judge) within run-to-run noise. The 9-judge ensemble achieves κ = 0.49 against published NIST qrels on TREC RAG 2024 (537 stratified-balanced pairs; §7), so the convergence is not specific to our institutional corpus.

**C2. Within-family agreement is bounded by the cross-family ceiling.** Three of four within-family pairs are at or below the strongest cross-family commercial pair (0.79): Anthropic 0.71, OpenAI 0.63, Google-commercial 0.67. The fourth, cross-organization Open-weight (Qwen ↔ Gemma 4 = 0.80), is the matrix maximum. **No commercial within-family pair dominates cross-family agreement** — at odds with self-preference findings on open-ended generation [panickssery2024self]. Our companion 4-way extraction study reports within-family ~2× cross-family on open-vocabulary tasks; the inversion under bounded ordinal judging suggests self-preference may be mediated by output-space boundedness rather than provider lineage (we discuss this hypothesis in §9).

**C3. Open-weight judges produce the matrix-highest within-pair κ and cluster with strict-mid commercial models.** Qwen ↔ Gemma 4 = **0.80** — exceeds every commercial within-family pair and ties the cross-family reasoning ceiling. Both open-weight judges (mean 1.11-1.18) cluster with GPT-4o and Opus 4.7 (1.09-1.15) **not** with the reasoning-generous triad (1.63-1.68). At ~USD 0.30 marginal cost via API routing, **an open-weight cross-organization pair achieves higher cross-validation κ than the ~USD 18 commercial 7-judge frontier**; we note that this measurement uses API routing rather than self-hosted inference, so on-prem economics are an extrapolation pending direct replication. External corroboration: Qwen and Gemma 4 both achieve **100% coverage** on TREC RAG 2024 (better than 4 of 7 frontier judges), with individual κ vs human qrels of 0.41 each.
...
# 6. Mechanism + bias diagnostics (Summary)

**Decomposition of κ.** A joint-distribution decomposition of κ into dispersion (the quadratic-disagreement weight, which appears in the κ formula by construction) and effective rank (a singular-value summary of the 4×4 confusion matrix) recovers R² = 0.928 across the 36 unique judge pairs. The high R² with dispersion is *partly mathematical* — synthetic-pair simulation confirms R² ≈ 0.998 in the limit purely from the κ formula's weighting structure — so we present this as a **decomposition**, not as the discovery of a hidden empirical relationship. Marginal-distribution KL divergence [kullback1951] gives R² = 0.33 as a coarser proxy. **The empirically interesting result is that structural factors (provider, reasoning-mode, model class) add ≤ 0.005 incremental R² above the decomposition and are individually non-significant** (all p > 0.18) — descriptively consistent with full mediation in the spirit of [baronkenny1986; mackinnon2007], though n = 36 limits the inferential strength. The **shared-tokenizer hypothesis is refuted**: Qwen ↔ Gemma 4 vocabulary Jaccard = 0.066 (lowest in slate) yet κ = 0.80 (matrix-highest); convergence operates at the decision-making layer, not the tokenizer. We use Cohen's κ [cohen1968] in its original quadratic-weighted formulation per Fleiss [fleiss1971] with Landis-Koch [landis1977] bands. **Bias and self-consistency diagnostics**: a 5×5 family conditional-mean matrix shows calibration drift, not self-preference (no family's diagonal dominates its row); intra-judge κ across 3 runs of each judge on 50 pairs averages **0.93 (range 0.89–1.00)**, with **all 9 judges more self-consistent than human-aligned** (Δ > 0); open-weight judges (Qwen 0.89, Gemma 0.91) are the *least* self-consistent in the slate, supporting an ensemble (not single-call) deployment recommendation. Full analysis: companion `long.md` §6.
...
**Results.** Per-judge κ vs human qrels ranges 0.40-0.55; **9-judge ensemble median κ = 0.4941, 95% CI [0.43, 0.56]** (1000-resample bootstrap, seed = 42), **7-judge frontier-only κ = 0.5187 [0.46, 0.58]** (open-weight broadens coverage, slightly lowers κ — a robustness/headline tradeoff). 5 of 9 judges hit κ ≥ 0.47. **Both ensembles outperform an UMBRELA single-judge baseline** (GPT-4o + UMBRELA prompt [upadhyay2024umbrela], same 537 pairs, κ = 0.4265) by 0.07-0.09 κ, supporting the methodology contribution as cross-family ensemble (not prompt) advance. Compared to Thakur et al.'s GPT-4o κ = 0.60 [thakur2025trecragsupport], our GPT-4o = 0.41 — gap likely from sample (ours stratified-balanced; theirs natural-proportional), prompting, qrels version.

**Third-corpus replication on TREC-COVID** (300 biomedical-scientific pairs, BEIR-distributed) reaches 9-judge ensemble κ = **0.3447** (fair-near-moderate), with Opus 4.7 leading at 0.53 and the frontier-7 subset at 0.4462; Qwen + Gemma 4 broaden coverage to 100% but trail their TREC RAG 2024 numbers (0.32/0.27 vs 0.41/0.40), indicating the robustness↔headline-κ tradeoff is content-domain dependent. **Coverage divergence is a content-domain reliability axis.** On TREC RAG 2024, Anthropic + OpenAI + Qwen + Gemma all return scores on **100%** of pairs; **Gemini 3.1 Pro Preview 24%, Gemini 2.5 Pro 17%** (thinking-mode parse aborts on long web-domain passages), DeepSeek V4 Pro 40%. On BEIR scifact (300 pairs, all-positive qrels — κ undefined; 9-judge ensemble precision at ≥2 = **65.7%**, range 43-75%), the same Gemini judges hit 60-86% — reliability depends on content character. The **always-works 6-judge subset** (≥95% across all three external corpora) is Anthropic + OpenAI + Qwen + Gemma — **the 2 cheapest judges make this subset**. Validation pipeline replaces a per-institution IRB study (~$1,500-3,000, 6-12 weeks) with public-NIST evidence (~$56, ~13 h). Per-corpus tables: `PUBLIC_BENCHMARKS_VALIDATION.md`.
